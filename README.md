...
>
>
> HuggingFace에 모델 배포 : https://huggingface.co/lhj815
...
- [사용 기술 스택](#사용-기술-스택)
- [프로젝트 설계, 구현](#프로젝트-설계-구현flowchart)
- [주요기능별 코드 정리, 상세](#주요기능별-코드-정리-상세-pdf-파일)
- [설치 방법](#설치-방법)
...
│ CSV 업로드 or data/ 선택           │
│ → read_csv_flexible              │
│ → 여러 CSV 병합(Optional source)  │
│ 자동 컬럼 매핑(auto_map_columns)   │
│ 날짜/판매량/지역/브랜드/상품 검출     │
└───────────────┬──────────────────┘
                │ (df, mapping 생성)
                ▼
...
│        ③ 예측·발주 탭              │
├──────────────────────────────────┤
│ 세그먼트 선택(region/brand/item)   │
│ 모델 및 feature_names 로드         │
│ iterative_forecast() 반복 예측     │
│ 가격 자동 인식                      │
│ 재고 자동 인식                      │
│ 총 수요 / 총 매출 예측              │
│ 발주량 계산 (예측–재고)             │
...
│      ⑤ 진단/로그 탭                │
├──────────────────────────────────┤
│ data/artifacts/models 파일 목록    │
│ ngrok/cloudflared 퍼블릭 URL      │
│ 터널 열기/닫기                     │
└──────────────────────────────────┘
